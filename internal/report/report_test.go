...
func TestWriteJSONStableFieldNames(t *testing.T) {
	...
	// Compare field names rather than exact bytes to be robust to formatting.
	for _, field := range []string{"mutation_id", "file", "line", "column", "operator", "original", "mutant", "status", "killing_tests"} {
		if !strings.Contains(string(golden), `"`+field+`"`) {
			t.Errorf("golden file missing field %q", field)
		}
	}
