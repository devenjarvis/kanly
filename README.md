...
- **Narrow operator surface.** Today Kanly mutates arithmetic operators, integer comparisons, boolean logic, `error`-typed return values, and void-discarded call statements. Code dominated by string templates (e.g. `internal/schema/template.go`), JSON construction, or slice/map indexing still emits few or no mutants. The highest-leverage operator families still to add: string-literal mutation and slice/map index mutation. Deleting side-effecting calls (e.g. `close(ch)`, locks, or sync primitives) can cause hangs in concurrent code; the per-mutant `--timeout` bounds this and surfaces them as `Timeout` rather than `Survived`.
